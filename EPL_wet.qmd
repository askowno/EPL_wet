...
wet <- wetlands %>%
  ...
  # clean up tiny types and remove estuaries
  filter(T_BIOREGIO != "Estuary") %>%
  # re code blank (not blank but one space actually) PES 2022 and 2018 scores to "Data Deficient" [DD] 
...
# make a simple table of TYPE and HGM to join with results later 
  type_hgm_tb <- wet %>% select(TYPE, HGM_4A) %>% distinct(TYPE, .keep_all = TRUE) %>% st_drop_geometry()
...
wet_type_18 <- wet_tb %>%
  ...
  summarise(extent = sum(as.numeric(Shape_Area), na.rm = TRUE)) %>%
  ungroup() %>%
