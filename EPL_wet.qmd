...
# Read PAs (projected layer)
pa <- st_read("C:/Users/skownoa/Dropbox/NBAwork/NLC_change/NLC_Change.gdb",
              layer = "SANBI_PA_2024Q4_Mar2025")

# Read Wetlands (unprojected layer)
wetlands <- st_read("C:/Users/skownoa/Dropbox/NBAwork/Wetlands/wetl_NBA_20250823.gpkg",
                    layer = "wetl_nba_20250823")

# Reproject wetlands to PA's CRS
wetlands <- st_transform(wetlands, crs = st_crs(pa))

# add fid for each feature and ensure area_m2 is correct
wetlands <- wetlands %>% mutate(fid = row_number()) %>% mutate(area_m2 = as.numeric(st_area(geom))) 
...
## Clean up this version 1) fill in missing type names by combining "Bioregion" and "_" and "hgm1" , and EC18VEG has a few "AA" change to "A". 
wet <- wetlands %>%
  # fix error in EC18VEG
  mutate(EC18VEG = ifelse(EC18VEG == "AA", "A", EC18VEG)) %>%
  # fix NA in type by combing bior and hgm1
  mutate(type = ifelse(type == "" | is.na(type), str_c(bioregion, hgm1, sep = "_") , type)) %>%
  # limit max PES to 100 and min to 0 (to ensure later use of values works) 
  mutate(across(starts_with("PES"), ~ round(pmin(pmax(., 0), 100)))) %>%
# drop M and Z coordinates for later computations like extract and hulls
